...
| Hosted runtime | Cloudflare Worker through `@cloudflare/vite-plugin` and Wrangler | Plugin `1.37.1`, Wrangler `4.92.0` |
| PWA | Web App Manifest and custom service worker | Cache name `midori-kanjo-v4` |
...
| Legacy desktop scaffold | Electron/electron-builder files under `desktop/` | Retained and used by the one-test desktop shell check; the active installer workflow is Tauri |
...
- Android Gradle tooling; the documented local requirement is Android SDK 34 and Java 17.
...
| Android public release | Debug packaging, release bundle command and tests exist | A Play Store build still requires the owner's signing key and store account |
| Desktop release | Tauri installer workflow exists and packaging tests pass | Installers are not commercially code-signed or Apple-notarized; physical Windows/Mac, printer, firewall/proxy, SmartScreen and Gatekeeper checks remain manual |
| Current phone release | Implementation, automated checks and responsive browser audit are complete | The nine-file mobile change set is uncommitted, unpushed and unpublished |

## 7. Exact files created or modified, with the purpose of each file

The current mobile pass modified exactly these tracked files relative to `origin/main`:
...
| `tests/mobile-package.test.mjs` | Modified | Expands the suite from seven to ten tests and asserts five tabs, touch targets, safe areas, zoom and landscape support |

This handoff adds one new file:

| File | Status | Purpose |
| --- | --- | --- |
| `docs/CHAT_HANDOFF.md` | Created | Complete continuation context for a new developer or ChatGPT chat |

No other tracked source files were modified by the current phone pass, and there were no other untracked files before this handoff was added.
...
Every table has a stable text `id` primary key and a `business_id` derived from the authenticated anonymous user's `sync_code`. RLS is enabled for every table. Policies permit authenticated operations only when `business_id` equals the JWT sync code and the code length is at least 20 characters.
...
`supabase/schema.sql` is the consolidated schema for a fresh project. Enable anonymous sign-in before using it.

Important migration caveat: existing cloud rows whose `business_id` is null remain deliberately hidden after the secure-business migration. The recorded migration instruction is to export and re-import those rows under the chosen sync code, not to weaken the policies.

### Synchronization rules

- IndexedDB is the source of truth for live counter operation.
- Cloud configuration may be entered in **More → Cloud backup**. A saved browser configuration takes precedence over build-time variables.
- A sync code must be at least 20 characters.
- Supabase URLs must use HTTPS, except localhost during development.
...
- Pushes use idempotent `upsert(..., { onConflict: "id" })` in batches of 100.
...
The same three values can instead be entered at runtime through Cloud backup settings. Do not commit `.env.local`.
...
No known functional regression is currently failing an automated test, and the final responsive audit reported no console errors. The verified unresolved items are:

1. **Release state:** the mobile optimization is still uncommitted, unpushed and unpublished.
2. **Large JavaScript bundle:** Vite warns that the main mobile JavaScript chunk is `857.58 kB` minified (`239.86 kB` gzip), above the 500 kB warning threshold. The hosted build also emits a large-chunk warning. This is an optimization issue, not a current build failure.
3. **Stale README test count:** `README.md` still calls the core suite a 21-test suite, while `tests/billing-core.test.ts` now contains and passes 33 tests.
...
5. **Existing pre-migration cloud data:** rows with null `business_id` are hidden by the secure RLS migration and require export/re-import under a valid sync code.
6. **Native signing/distribution:** Android store signing, Windows commercial code-signing and Apple notarization are not complete. SmartScreen/Gatekeeper warnings are expected for internal unsigned builds.
7. **Manual hardware acceptance:** the repository explicitly records that CI cannot validate physical printers, shop firewall/proxy behavior, SmartScreen policy or Gatekeeper policy.
8. **Real native cloud round-trip:** the test workflow exists, but this handoff session did not run `test:tauri:e2e` because it requires the dedicated Supabase test variables. Do not claim it ran unless a workflow/test log is available.
9. **Physical phone evidence:** the verified final mobile audit used responsive device dimensions and interaction checks. The conversation does not establish a separate test on a named physical Android handset or iPhone.
...
- Lint, 33 core tests, 10 mobile tests and five combined desktop/Tauri package tests pass.
...
Build a debug APK on a machine with Android Studio, SDK 34 and Java 17:
...
The standard local Tauri build uses the targets in `src-tauri/tauri.conf.json`. The GitHub workflow is the verified route for Windows x64 MSI/NSIS EXE and universal macOS DMG artifacts.
...
4. Enter the project URL, anon key and a private sync code of at least 20 characters in **More → Cloud backup**, or use the documented environment-variable names.
...
## 17. Tests already performed and their results

The following were rerun against the exact local mobile change set while producing this handoff:

| Command/check | Result |
| --- | --- |
| `npm run lint` | Passed, exit code 0 |
| `npm run test:unit` | 33 passed, 0 failed |
| `npm run build` | Passed; validated Cloudflare Worker `default.fetch` and packaged hosting manifest |
| `node --test tests/rendered-html.test.mjs` | 1 passed, 0 failed |
| `npm run test:mobile` | 10 passed, 0 failed; Capacitor sync completed |
| `npm run test:desktop` | 1 passed, 0 failed |
| `npm run test:tauri:config` | 4 passed, 0 failed |
| `git diff --check` | Passed; no whitespace errors |

The 33 core tests include billing, GST, units, PDF formats, charges, receivables/payables, dues, expenses, reports, quotation isolation, owner PINs, drafts, merges, daily close, paginated 2,501-item sync, upload batching and ledger reconciliation.

The ten mobile tests include the offline bundle, branding, PWA manifest/service worker, five-tab layout, More shortcuts, touch targets, landscape rules, zoom/orientation, embedded Android assets, native plugins/permissions and secure business sync.

The five combined desktop/Tauri package tests include the hardened legacy shell check, Tauri identity/configuration, icons/window persistence, exclusion of the test harness from production and both requested installer artifact groups in GitHub Actions.
...
- Vite/Vinext reports JavaScript chunks above 500 kB; the largest verified mobile chunk is `857.58 kB` minified.
...
6. Ask for or confirm authorization before GitHub publication. Then commit the nine mobile files plus this handoff intentionally and push `main`.
...
10. Address the large mobile bundle warning and stale README test-count line as follow-up maintenance.
